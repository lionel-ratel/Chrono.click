...
$cck		=	JCckDev::preload( array( 'core_filter_input', 'core_filter_go', 'core_filter_search', 'core_filter_clear',
										 'core_state_filter', 'more_toolbox_job_type' ) );
...
<div class="<?php echo $this->css['items']; ?>">
	...
	foreach ( $this->items as $i=>$item ) {
		...
		$file			=	( $item->type == 'cron_cli' && is_file( JPATH_SITE.'/cli/cck_job_'.$item->name.'.php' ) ) ? 'cli/cck_job_'.$item->name.'.php' : '';
		...
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td class="center hidden-phone"><?php echo ( !$item->type ) ? Text::_( 'COM_CCK_MANUAL' ) : Text::_( 'COM_CCK_'.$item->type ); ?></td>
...
<div>
	...
	<input type="hidden" name="return_v" id="return_v" value="jobs" />
