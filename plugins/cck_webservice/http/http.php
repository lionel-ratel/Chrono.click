...
// Plugin
class plgCCK_WebserviceHttp extends JCckPluginWebservice
{
	...
	public function onCCK_WebserviceCall( &$webservice, $fields, $config = array() )
	{
		...
		if ( isset( $webservice->params ) ) {
			$options2	=	new Registry( $webservice->params );
		} else {
	...
	protected function _prepare( &$webservice, $options, $options2, $fields )
	{
		...
		if ( $method == 'post' || $method == 'put' ) {
			...
			$vars				=	'';
			self::_setVars( $vars, $format, $webservice, $fields );
			$settings			=	array(
										CURLOPT_POST=>1,
			...
			$vars				=	'';
			self::_setVars( $vars, 'nvp', $webservice, $fields );
			if ( $vars ) {
	...
	// _setVars
	protected static function _setVars( &$vars, $format, $webservice, $fields )
	{
		$isMultisite	=	false;
		$params			=	array();

		if ( $webservice->request_options !== '' ) {
			$params		=	json_decode( $webservice->request_options, true );
		}

		// Protect Factory::getApplication for CLI
		try {
			if ( Factory::getApplication()->isClient( 'site' ) ) {
				$isMultisite	=	JCck::isSite();
			}
		} catch ( Exception $e ) {
		}
