<?php
/**
* @version 			SEBLOD WebServices 1.x ~ $Id: pkg_script.php sebastienheraud $
...
// Script
class pkg_cck_webservicesInstallerScript
{
	// install
	public function install( $parent )
	{
		$data	=	"<!DOCTYPE html><title></title>";
		$groups	=	array( 'cck_webservice' );
		foreach ( $groups as $group ) {
			File::write( JPATH_PLUGINS.'/'.$group.'/'.'index.html', $data );			
		}
	}
	...
	public function postflight( $type, $parent )
	{
		...
		$db->setQuery( 'UPDATE #__extensions SET enabled = 1 WHERE type = "plugin" AND element = "cck_webservice"' );
		$db->execute();

		$params =	ComponentHelper::getParams( 'com_cck_webservices' );
		$root	=	JPATH_ADMINISTRATOR.'/components/com_cck_webservices/';
		
		// SQL
		if ( $type == 'install' ) {
			// Set Initial Version
			$params->set( 'initial_version', $app->cck_core_version );
			$db->setQuery( 'UPDATE #__extensions SET params = "'.$db->escape( $params ).'" WHERE name = "com_cck_webservices"' );
			$db->execute();

			// Publish Plugins
			$query	=	'UPDATE #__extensions SET enabled = 1 WHERE folder = "cck_webservice"';
			$db->setQuery( $query );
			$db->execute();
		} else {
