...
// Plugin
class plgCCK_Field_RestrictionJoomla_User_Group extends JCckPluginRestriction
{
	...
	protected static function _authorise( $restriction, &$field, &$config )
	{
		...
		$groups			=	$restriction->get( 'groups', '0' );
		$groups			=	explode( ',', $groups );
		$user 			=	JCck::getUser();
		$user_groups	=	$user->groups;

		// Prepare
		if ( !( count( array_diff( $groups, $user_groups ) ) < count( $groups ) ) ) {
