...
class SmartSearchItemType
{
    ...
    public static function metaString($item, array $args): string
    {
        ...
        foreach ($props as $field => $prop) {
            $article->$prop = $item->getElement(is_numeric($field) ? $prop : $field);
        }
     ...
     * @param Result $item
     */
    public static function category($item): ?CategoryNode
    {
     ...
     * @param Result $item
     */
    public static function author($item): User
    {
        $user = UserHelper::get((int) $item->getElement('created_by'));
