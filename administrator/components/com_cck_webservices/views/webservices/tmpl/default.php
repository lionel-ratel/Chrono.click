...
<div class="<?php echo $this->css['items']; ?>">
	...
	<thead>
		<tr>
			...
			<th width="20%" class="center hidden-phone nowrap"><?php echo HTMLHelper::_( 'grid.sort', 'COM_CCK_TYPE', 'a.type', $listDir, $listOrder ); ?></th>
	...
	foreach ( $this->items as $i=>$item ) {
		...
		require_once JPATH_SITE.'/plugins/cck_webservice/'.$item->type.'/'.$item->type.'.php';
		$url			=	JCck::callFunc( 'plgCCK_Webservice'.$item->type, 'getInfoURL', $item );
		?>
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td width="30px" class="center hidden-phone dropdown-col">
				<?php
				if ( $url ) {
					HTMLHelper::_( '.cckactionsdropdown.addCustomLinkItem', Text::_( 'COM_CCK_FILTER' ), 'play', 'cb_link'.$i, $url );
            ...
            <td class="center hidden-phone"><?php echo Text::_( 'PLG_CCK_WEBSERVICE_'.$item->type.'_LABEL2' ); ?></td>
...
<div>
	...
	<input type="hidden" name="return_v" id="return_v" value="webservices" />
