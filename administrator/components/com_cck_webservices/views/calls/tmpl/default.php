...
$title2		=	Text::_( 'COM_CCK_CALL_THE_WEBSERVICE' );
$config		=	JCckDev::init( array( '42', 'button_submit', 'select_dynamic', 'select_simple', 'text' ), true, array( 'vName'=>$this->vName ) );
$cck		=	JCckDev::preload( array( 'core_filter_input', 'core_filter_go', 'core_filter_search', 'core_filter_clear', 'core_state_filter' ) );
...
<?php include_once __DIR__.'/default_definitions.php'; ?>
...
<div class="<?php echo $this->css['items']; ?>">
	...
	<thead>
		<tr>
			...
			<th width="20%" class="center hidden-phone nowrap"><?php echo HTMLHelper::_( 'grid.sort', 'COM_CCK_WEBSERVICE', 'webservice_title', $listDir, $listOrder ); ?></th>
			<th width="22%" class="hidden-phone nowrap"><?php echo HTMLHelper::_( 'grid.sort', 'COM_CCK_CALL_REQUEST', 'a.request', $listDir, $listOrder ); ?></th>
	...
	foreach ( $this->items as $i=>$item ) {
		...
		$link2 			=	Route::_( 'index.php?option='.$this->option.'&task='.$this->vName.'.call&layout=call&id='. $item->id );
		?>
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td width="30px" class="center hidden-phone dropdown-col">
				<?php
				if ( $link2 && $item->standalone ) {
					HTMLHelper::_( '.cckactionsdropdown.addCustomLinkItem', Text::_( 'COM_CCK_EXECUTE' ), 'play', 'cb_link'.$i, $link2 );
            ...
            <td class="center hidden-phone"><?php echo $item->webservice_title; ?></td>
            <td class="hidden-phone">
            	<?php
            	$method	=	$item->request_method ? $item->request_method : $item->webservice_options->get( 'method' );

            	if ( ( $pos = strpos( $item->request, '$cck->get' ) ) !== false ) {
            		$item->request	=	substr( $item->request, 0, $pos ).'$cck->{...}';
            	} elseif ( ( $pos = strpos( $item->request, '$uri->get' ) ) !== false ) {
            		$item->request	=	substr( $item->request, 0, $pos ).'$uri->{...}';
            	}
            	echo strtoupper( $method ).' '.$item->request;
            	?>
            </td>
	...
	<tfoot>
		<tr height="40px;">
			...
			<td colspan="6" id="pagination-bottom"><?php echo $this->pagination->getListFooter(); ?></td>
			<td class="center hidden-phone"><?php Helper_Display::quickSlideTo( 'border-top', 'up' ); ?></td>
		</tr>
	</tfoot>
	<?php } ?>
...
<div>
	...
	<input type="hidden" name="return_v" id="return_v" value="calls" />
