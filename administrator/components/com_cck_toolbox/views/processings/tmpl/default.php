...
$action			=	'<span class="icon-play"></span>';
$action_attr	=	' class="btn btn-primary btn-micro hasTooltip" title="'.Text::_( 'COM_CCK_EXECUTE_THE_SCRIPT' ).'"';
$css			=	array();
...
$title2			=	Text::_( 'COM_CCK_EXECUTE_THE_SCRIPT' );
...
$cck			=	JCckDev::preload( array( 'core_filter_input', 'core_filter_go', 'core_filter_search', 'core_filter_clear',
										 	 'core_state_filter', 'more_toolbox_processing_type' ) );
...
<div class="<?php echo $this->css['items']; ?>">
	...
	<thead>
		<tr>
			...
			<th width="20%" class="center hidden-phone nowrap"><?php echo HTMLHelper::_( 'grid.sort', 'COM_CCK_TYPE', 'a.file', $listDir, $listOrder ); ?></th>
	...
	foreach ( $this->items as $i=>$item ) {
		...
		$file			=	( $item->scriptfile[0] == '/' ) ? substr( $item->scriptfile, 1 ) : $item->scriptfile;
		$link 			=	Route::_( 'index.php?option='.$this->option.'&task='.$this->vName.'.edit&id='. $item->id );
		$link2 			=	Route::_( 'index.php?option='.$this->option.'&task='.$this->vName.'.process&layout=process&id='. $item->id );
		...
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td width="30px" class="center hidden-phone dropdown-col">
				<?php
				if ( !$item->type ) {
					HTMLHelper::_( '.cckactionsdropdown.addCustomLinkItem', Text::_( 'COM_CCK_EXECUTE_THE_SCRIPT' ), 'play', 'cb_link'.$i, $link2 );
			...
			<td>
				<div class="title-left" id="title-<?php echo $item->id; ?>">
					...
					if ( $canEdit && ! $checkedOut ) {
						echo '<a href="'.$link.'">'.$this->escape( $item->title ).'</a><div class="small">'.$this->escape( $file ).'</div>';
					} else {
						echo '<span>'.$this->escape( $item->title ).'</span><div class="small">'.$this->escape( $file ).'</div>';
					}
					?>
				</div>
			</td>
			...
			<td class="center hidden-phone"><?php echo ( !$item->type ) ? Text::_( 'COM_CCK_MANUAL' ) : $item->type; ?></td>
	...
	<tfoot>
		<tr height="40px;">
			...
			<td class="center" colspan="7" id="pagination-bottom"><?php echo $this->pagination->getListFooter(); ?></td>
			<td class="center hidden-phone"><?php Helper_Display::quickSlideTo( 'border-top', 'up' ); ?></td>
		</tr>
	</tfoot>
	<?php } ?>
...
<div>
	...
	<input type="hidden" name="return_v" id="return_v" value="processings" />
