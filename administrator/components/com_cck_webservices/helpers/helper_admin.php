...
class Helper_Admin extends CommonHelper_Admin
{
	...
	public static function addToolbar( $vName, $vTitle ) 
	{
		...
		if ( $canDo->get( 'core.create' ) || $canDo->get( 'core.edit' ) ) {
			if ( $canDo->get( 'core.create' ) ) {
				if ( $vName == 'call' ) {
					ToolbarHelper::custom( $vName.'.add', 'new', 'new', 'COM_CCK_CALL', false );
					ToolbarHelper::custom( 'webservice.add', 'new', 'new', 'COM_CCK_WEBSERVICE', false );
				} elseif ( $vName == 'integration' ) {
					ToolbarHelper::custom( 'integration_app.add', 'new', 'new', 'COM_CCK_INTEGRATION_APP', false );
					ToolbarHelper::custom( 'integration_auth.add', 'new', 'new', 'COM_CCK_INTEGRATION_AUTH', false );
				} else {
	...
	public static function addToolbarEdit( $vName, $vTitle, $vMore = '', $params = array() ) 
	{
		...
		if ( isset( $vMore['layout'] ) && $vMore['layout'] == 'call' ) {
			ToolbarHelper::title( Text::_( _C2_TEXT ).': <small><small>[ '.Text::_( 'COM_CCK_CALL' ).' ]</small></small>', $class );
	...
	// addToolbarDelete
	public static function addToolbarDelete( $vName, $vTitle, $folderId = 0 ) 
	{
		Factory::getApplication()->input->set( 'hidemainmenu', true );
